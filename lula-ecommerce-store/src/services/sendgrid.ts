...
export class SendGridService extends NotificationService {
    ...
    switch (event) {
      ...
      case "order.return_requested": {
        ...
        if (shipping_method) {
          ...
          const lbl: FromFullFilementService[] =
  ...
  async sendNotification(
    ...
    if (data.locale) {
      const localizedTemplateId = this.getLocalizedTemplateId(
        event,
        data.locale
      );
      const localizedSubject = this.getLocalizedTemplateId(
        event,
        data.locale,
        true
      );
    ...
    const sendOptions: SendGrid.MailDataRequired = {
      ...
      to: data.email,
      bcc:
        this.options_.orderPlacedBcc && event === "order.placed"
          ? this.options_.orderPlacedBcc
          : undefined,
  ...
  async orderShipmentCreatedData(
    { id, fulfillment_id }: EventData,
    attachmentGenerator?: any
  ) {
